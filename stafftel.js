...
const AIRTABLE_BASE  = process.env.AIRTABLE_BASE_ID || 'appGrgwkoEmYyWHLn';
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'load_dashboard') {
      ...
      // ── Mapping colonnes réelles Airtable ──
      const staff = staffData.records.map(r => ({
        id:     r.id,
        nombre: r.fields['NOMBRE']           || '',
        rol:    r.fields['ESPECIALIDAD']     || '',
        // DISPONIBLE = checkbox true/false
        statut: r.fields['DISPONIBLE'] === false ? 'Ausente' : 'Disponible',
        carga:  r.fields['CARGA_SEMANA_HORAS'] || 0,
      }));

      const rooms = roomsData.records.map(r => ({
        id:        r.id,
        numero:    r.fields['# Chambre']        || '',
        etage:     r.fields['# Étage']          || '',
        tipo:      r.fields['Type']             || '',
        statut:    r.fields['Statut']           || '',
        priorite:  r.fields['Priorité']         || '',
        nettoyage: r.fields['Nettoyage_Requis'] || false,
        employe:   r.fields['Employé']          || '',
      }));

      const planning = planningData.records.map(r => ({
        id:          r.id,
        staffNombre: Array.isArray(r.fields['Assignee'])
          ? r.fields['Assignee'].map(a => a.name || a.email || '').join(', ')
          : (r.fields['Assignee'] || ''),
        habitacion:  r.fields['Name']   || '',
        horario:     '',
        tarea:       r.fields['Notes']  || '',
        statut:      r.fields['Status'] || '',
      }));

      const staffDisponible = staff.filter(s => s.statut === 'Disponible').length;

      // Temps total : somme CARGA_SEMANA_HORAS du staff disponible
      const tiempoTotal = staff
        .filter(s => s.statut === 'Disponible')
        .reduce((acc, s) => acc + (Number(s.carga) || 0), 0) * 60;

      // Occupation : chambres dont Statut = 'Ocupada'
    ...
    if (action === 'reportar_ausencia') {
      ...
      const r = await fetch(
        `https://api.airtable.com/v0/${AIRTABLE_BASE}/${TABLES.staff}/${staffId}`,
        {
          ...
          body: JSON.stringify({ fields: { DISPONIBLE: false } })
        }
      );
      ...
      const n8nRes = await fetch(N8N_WEBHOOK, {
        ...
        body: JSON.stringify({
          ...
          staff: staffData.records.map(r => ({
            id: r.id, nombre: r.fields['NOMBRE'] || '', rol: r.fields['ESPECIALIDAD'] || '', disponible: r.fields['DISPONIBLE'] !== false
          })),
          rooms: roomsData.records.map(r => ({
            id: r.id, numero: r.fields['# Chambre'] || '', etage: r.fields['# Étage'] || '', tipo: r.fields['Type'] || '', statut: r.fields['Statut'] || '', nettoyage: r.fields['Nettoyage_Requis'] || false
          })),
        })
      });
